...
const countNumberOfElements = (arr) => {
  return arr.reduce((numOne, value, idx) => {
    numOne++;
    return numOne;
  }, 0);
};
...
const returnNames = (arr) => {
  let numTwo = [];
  arr.reduce((someNum, val) => {
    numTwo.push(val.name)
  }, 0);
  return numTwo;
};
...
const reversedString = (str) => {
  return str.split('').reduce((numThree, val, idx) => {
    numThree = `${val}${numThree}`;
    return numThree;
  });
};
...
const countNumberOfChildren = (arr) => {
  return arr.reduce((numFour, child, idx) => {
    if (child.children)
      numFour += child.children.length;
    return numFour;
  }, 0);
};
...
const calculateAverage = (arr) => {
  let numFive = 0;
  arr.reduce((valOne, valTwo) => {
    numFive += valTwo;
  }, 0);
  return (numFive / (arr.length));
};
